"use client"
import { BACKEND } from "@/app/config/config";
...
export const LoginForms = () => {
	...
	const handleLogin = async (data:any) => {
		try{
			const res = await fetch(`${BACKEND}/api/v1/auth/login`, {method:"POST", body:JSON.stringify(data), headers:{"Content-Type":"application/json"}});
  ...
  return (
    <div className="bg-[#222533] h-96 w-[320px] p-5 rounded-2xl">
      ...
      <form className="flex flex-col gap-2 mt-12" onSubmit={handleSubmit(handleLogin)}>
        <p>Login</p>
        <input
		{...register("email")}
